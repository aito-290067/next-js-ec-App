  ...
  return (
    ...
    <div className="w-48 h-80  rounded-lg   shadow-lg  cursor-pointer">
      ...
      <h5 className="my-3 text-lg font-bold text-gray-500 dark:text-gray-400 text-center" >
        {Number(props.price).toLocaleString()}円<span className="text-sm font-light">（税込）</span>
      </h5>
    ...
    <div className=" w-[150px]   h-[250px] sm:h-[250px] cursor-pointer mx-1 shadow-md rounded-md">
      ...
      <div className="flex">
        ...
        <div className="px-6  sm:px-4 ">
          <Link href={`/items/${props.id}`}>
            <a>
              <p className={`mb-2 text-sm pt-0.5 text-gray-500 ${style.recognizeItemName}  sm:h-10 h-10 overflow-hidden`}>
                {props.name}
              </p>
            </a>
          </Link>
      ...
      {Number(props.price).toLocaleString()}円<span className="block sm:inline-block text-[10px] hidden">（税込）</span>
      </h5>
