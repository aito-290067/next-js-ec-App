...
export const ShoppingList = (props: any) => {
  ...
  //合計金額算出 
  let initTotalPrice = priceList.reduce(function (sum: number, element: number) {
      ...
      <div className="container flex flex-col justify-center items-center mx-auto py-5 px-5 ">

        {
          data && data.map((shoppingItems: {
            ...
            return (
              <div className={` mb-1 grid sm:gap-1 grid-cols-5 rounded-md md:w-[800px] 
                        `} key={index}>
                <ItemCardsSide
        ...
        <div className=" mt-10 mb-5 px-5 ">
          <p className="text-sm">合計金額&nbsp;&nbsp;&nbsp;<span className="text-[35px]  text-[#75ad9d]  ">{totalPrice.toLocaleString()}</span> &nbsp;円(税込)</p>
        </div>
